...
const TeamDash = () => {
    ...
    const [problemSelects, setProblemSelects] = useState(Array(teams.length).fill(''));
    ...
    useEffect(() => {
        const getTeams = async () => {
            try {
                ...
                let res = await fetch(`${import.meta.env.VITE_BACKEND_URL}/api/v1/teams`, {
                    method: "GET",
    ...
    const selectCheckedTeams = async () => {
        ...
        try{
            setLoading(true)
            setEmailSent(false)
            let res = await fetch(`${import.meta.env.VITE_BACKEND_URL}/api/v1/teams/select-teams`,{
                method : "PATCH",
                headers: {"content-type": "application/json"},
    ...
    const unSelectCheckedTeams = async () => {
        ...
        try{
            setLoading(true)
            setEmailSent(false)
            let res = await fetch(`${import.meta.env.VITE_BACKEND_URL}/api/v1/teams/unselect-teams`,{
                method : "PATCH",
                headers: {"content-type": "application/json"},
                body: JSON.stringify(teamsToSend)
            });
    ...
    const getTeamDataInCSV = async () => {
        try {
            let res = await fetch(`${import.meta.env.VITE_BACKEND_URL}/api/v1/teams/csv`, {
                method: "GET"
            });
            if (res.status === 200) {
    ...
    const getUserDataInCSV = async () => {
        try {
            let res = await fetch(`${import.meta.env.VITE_BACKEND_URL}/api/v1/users/get_user_csv`, {
                method: "GET"
            });
            if (res.status === 200) {
                const blob = await res.blob();
    ...
    const handleCheckboxChange = (teamId) => {
      ...
      const assignProblemToTeam =  async (_teamId,index) => {
        ...
        try{
            ...
            let res = await fetch(`${import.meta.env.VITE_BACKEND_URL}/api/v1/teams/assign-problem`,{
                method: "PATCH",
                headers: {'Content-type': 'application/json'},
      ...
      const removeAssignedProblem = async(_teamId) => {
        ...
        try{
            let res = await fetch(`${import.meta.env.VITE_BACKEND_URL}/api/v1/teams/remove-assigned-problem`,{
                method: "PATCH",
                headers: {'Content-type': 'application/json'},
                body: JSON.stringify(payloadData)
        ...
        <>
            {!loading  ? (
                <>
                    <div className='container d-flex justify-content-center align-items-center' style={{color: "#fff"}}>
                        ...
                        <div className='row d-flex justify-content-center align-items-center'>
                            <h3 className='my-5' style={{color: "black"}}>Team Dashboard (MAJOR FIX REQUIRED 🔨)</h3>

                        </div>

                    </div>
                    <div className='d-flex justify-content-space-between align-items-center my-3'>
                    <button className='btn btn-success mx-5' onClick={getTeamDataInCSV}>Get Team Data in CSV</button>
                    <button className='btn btn-success mx-5' onClick={getUserDataInCSV}>Get User Data in CSV</button>
                    <button className='btn btn-primary mx-5' onClick={selectCheckedTeams} disabled = {!admin}>ShortList Teams</button>
                    <button className='btn btn-danger mx-5' onClick={unSelectCheckedTeams} disabled = {!admin}>Un-ShortList Teams</button>
                    <h4 className='btn btn-warning'>Total teams registered: {teams.length} <br/> Total users registered: {users.length} </h4>
                            ...
                            <>
                                {!team.isSelected?(
                                    ...
                                    <tr key={index}>
                                        ...
                                        <td><button type="button" onClick={() => getRespectiveTeamId(team.leader)} className="btn btn-dark" data-bs-toggle="modal" data-bs-target="#exampleModal">
                                            View Abstract
                                        </button>
                ...
                {!team.selectedProblem?(
                        ...
                        >                                                    <option selected value="">Assign a problem</option>
                        {problemsDropDown.map((problem,index) => (
                            <option key={index} value={problem._id}>{problem.title}</option>
                        ))}
                    </select>
                ):(
            ...
            <td>
                {team.selectedProblem?(
                    <button className='btn btn-danger' onClick={() => removeAssignedProblem(team._id)}>Remove</button>
                ):(
                    <button className='btn btn-primary' onClick={() => assignProblemToTeam(team._id, index)}>Assign</button>
                )}
            </td>
                                        </>
                                       ):<></>}
                                    ...
                                    <div className="modal-dialog" style={{ backgroundColor: "white" }}>
                                        <div className="modal-content">
                                            ...
                                            <div className="modal-body">
                                                <div className='d-flex' style={{ wordBreak: 'break-word' }}>
                                                    {problemLoader ? ("Fetching... Problems") : (
                                                        <ul>
                                                            {problems.map((prob, indx) => (
                                                                <>
                                                                    <li key={indx} className='my-2' ><b>Title: {prob.problemTitle}</b> <br></br> <b>Preference: {prob.problemPreference} </b> <br></br> <b>Abstract</b> :  - {prob.problemAbstract} <hr /> </li>
                                                                </>
                                                            ))}
                                                        </ul>
                                                    )}
